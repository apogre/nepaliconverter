...
public class Font2UnicodeMapping {
  ...
  int lineNr = 0;
    ...
    System.err.println(lineNr + " addLetter("+fontLetter+ "  "+unicLetter);
  ...
  HashSet<String> fontChars_NONSPAC = new HashSet<String>();
    ...
    if (fontChars_NONSPAC.size()==0) {
      ...
      for (F2Uelement e : fontLetter_to_F2UElement.values()) {
        if (e.unicLetter.length() == 1 && e.fontLetter.length() == 1 && NONSPAC.indexOf(e.unicLetter) != -1) {
          fontChars_NONSPAC.add(e.fontLetter);
      ...
      System.out.println("         Please add entry " +fontChars_NONSPAC.toString().replaceAll(", ","  ")+ " NONSPAC to spreadsheet");
  ...
  private String removeDuplicatesAndReorderNonspacingMarks(String input) {
      ...
      while (i < input.length()) {
          int j = i;
          while (j <= input.length() && fontChars_NONSPAC.contains(input.substring(j-1, j))) j++;
          if (j>i+1) {
              ...
              while (k<sb2.length()) {
                ...
                if (c0 == c1) {
                  sb2.deleteCharAt(k); // delete duplicate
                    ...
                    F2Uelement e0 = fontLetter_to_F2UElement.get(""+c0);
                    F2Uelement e1 = fontLetter_to_F2UElement.get(""+c1);
                    if (e0 == null) {
                        System.err.println("Spreadsheet error: This NONSPAC character is not mapping in mapping: " + c0);
                        k++;
                    } else  if (e1 == null) {
                        System.err.println("Spreadsheet error: This NONSPAC character is not mapping in mapping: " + c1);
                        k++;
                    } else if (e0.unicLetter.compareTo(e1.unicLetter)<0) {
