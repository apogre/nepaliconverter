...
*/
  ...
  public void reviziu() throws Exception {
    ...
    if (ordigu) {
      ...
      if (ordiguLauxRom) kromnomo = kromnomo + "LauxRom";
    }
    ...
    if (kromnomo.startsWith("-")) kromnomo = kromnomo.substring(1);

    if (kromnomo.length()==0) kromnomo = "reviziita";
...
//    Matcher m = Pattern.compile("(<text:p text:style-name=\"(\\w+)\">(.*?)</text:p>)").matcher(tuto);
    ...
    while (m.find()) {
      ...
      if (linio.endsWith("<text:tab/>")) linio = linio + " ";
	...
	continue;
      ...
      if (faruLigojn) esp = FaruLigojn.faruLigojn(esp);
      ...
      //System.out.println(rom + " / " + dev + " / " + esp + "   / "+m.start(2)+" " +m.end(2));
      if (faruLigojn) dev = FaruLigojn.faruNepalajnLigojn(dev);
	...
	Collections.sort(ne_eo, new Comparator() {
	  public int compare(Object o1, Object o2) {
	    String[] t1 = (String[]) o1;
	    String[] t2 = (String[]) o2;
	    if (ordiguLauxRom) return anglaCollator.compare(t1[0].toLowerCase().replace("v","b"), t2[0].toLowerCase().replace("v","b"));
	    //else return t1[1].compareTo(t2[1]);
	    else return hindiCollator.compare(t1[1], t2[1]);
